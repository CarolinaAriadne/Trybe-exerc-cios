...
let arrProdutos = []; 
techProducts.forEach(produto => { // percorre cada objeto (pega o objeto todo, com suas chaves)
  if ((produto.id % 2) === 0) { // se o elemento tiver um id de número par (divisível por 2)
...
let precoProduto = techProducts.some((element) => { // some: encontrar pelo menos um elemento que satisfaça a condição seguinte
    if (element.price > 300) { // se o elemento (retorna objeto todo), acessando chave price, for maior que 300
